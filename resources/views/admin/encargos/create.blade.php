...
<div class="row mx-0 mx-md-5 my-2">
    <h2 class="fuente-libre text-center">Añadir nuevo encargo</h2>
<form onsubmit="chispazoAbly2({'refreshPage':'refreshPage'});" action="{{route('encargos.storePanel')}}"  method="POST">
    @csrf
    <div class="d-flex flex-column justify-content-evenly mx-0 px-0 mx-md-5 px-md-5 fs-3">

        <div class="mx-5 px-5 my-2">
            <div class="form-group mb-2">
                <label for="name" class="form-label"><strong>Nombre:</strong></label>
                <input type="text" name="name" class="form-control fs-3" placeholder="Nombre" required>
            </div>
        ...
        <div class="mx-5 px-5 my-2">
                <div class="form-group mb-2">
                    ...
                    <input type="date" class="form-control fs-3" name="date" id="fecha_pedido" value="{{ date('Y-m-d') }}" min="{{ date('Y-m-d') }}" required>
                </div>

                <div class="form-group">
                    <label for="start_time"><strong>Hora de pedido:</strong></label>
                    <!-- <input type="time" class="form-control fs-3" name="hora_pedido" id="start_time" min="{{ date('H:i') }}" required> -->
                    <input type="time" class="form-control fs-3" name="hora_pedido" id="start_time" required>
                </div>
        </div>

        <div class="mx-5 px-5 my-2">
            <div class="form-group">
                <label for="description" class="form-label"><strong>Detalles del pedido:</strong></label>
                <textarea name="description" class="form-control fs-3" style="height:150px"
                    placeholder="¿Algo más?..." ></textarea>
            </div>
        </div>

        <div class="text-center mt-2">
            <input type="submit" class="btn btn-primary" value="Guardar" onclick="chispazoAbly2({'refreshPage':'refreshPage'})">
            <a class="btn btn-outline-primary ms-3" href="{{route('encargos.index')}}">Volver</a>
        </div>
