...
async function fetchBookingsWeekly9(accessToken) { 
    try {
        let allBookings = [];
        const oneDayInSeconds = 86400;
        ...
        async function fetchWithDynamicRange(startDate, endDate, rangeDescription = "rango original") {
            ...
            if (data.status === "SUCCESS" && data.data.bookings.length > 0) {
                ...
                if (data.data.bookings.length === 100) {
                    ...
                    const bookingsToSave = allBookings.map(booking => ({
                        short_id: booking.short_id,
                        product_name: booking.product_name,
                        supplier_company_name: booking.supplier_company_name,
                        seller_company_name: booking.seller_company_name,
                        ...
                        client_phone: booking.client_data?.phone || "N/A",
                        client_email: booking.client_data?.email || null,
                        client_id: booking.client_data?.id || null,
                        currency: booking.currency,
                        total_price: parseFloat(booking.total_price),
                        payment_partial: parseFloat(booking.payment_partial),
                        ticket_type_count: formatJSON(booking.ticket_type_count),
                        ...
                        status: booking.status,
                        source: booking.source,
                        pdf_download: booking.pdf_download || null,
                        verification_code: booking.verification_code || null,
                        verification_type: booking.verification_type || null
                    }));
        ...
        console.groupCollapsed(`📅 [Inicio de Rango] Procesando rango: ${new Date(startDate * 1000).toISOString()} a ${new Date(endDate * 1000).toISOString()}`);

        await fetchWithDynamicRange(startDate, endDate, "Últimos 30 días");

        console.groupEnd(`📊 [Acumulado Total] Total de datos acumulados: ${allBookings.length}`);
...
async function init() {
    ...
    try {
        ...
        await fetchBookingsWeekly9(accessToken);
    } catch (error) {
